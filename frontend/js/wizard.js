/**
 * wizard.js
 * The 5-step stepper: Family Details -> Address -> Members -> Review -> Submit.
...
import {
  el, toast, confirmDialog, newLocalId, isValidPhone, isValidEmail
} from './utils.js';

const STEP_TITLES = ['Family Details', 'Address', 'Family Members', 'Review', 'Submit'];
...
function blankFamily() {
  return {
    ...
    Remarks: '', District: '', State: '', Country: 'India', MapLink: ''
  };
}
...
function normalizeFamilyKeys_(target, source) {
  const map = {
    HouseNumber: 'houseNumber', FamilyName: 'familyName', HeadOfFamily: 'headOfFamily', Ward: 'ward',
    Area: 'area', HouseName: 'houseName', Address: 'address', Landmark: 'landmark', PinCode: 'pinCode',
    Phone: 'phone', WhatsApp: 'whatsapp', Email: 'email', Remarks: 'remarks', District: 'district',
    State: 'state', Country: 'country', MapLink: 'mapLink'
  };
...
function normalizeMemberKeys_(m) {
  return {
    MemberID: m.memberId || '', MemberName: m.name || '', Gender: m.gender || 'Male',
    Relation: m.relation || 'Head', DOB: m.dob || '', Age: m.age || '', WeddingDate: m.weddingDate || '',
    BloodGroup: m.bloodGroup || '', Phone: m.phone || '', WhatsApp: m.whatsapp || '', Email: m.email || '',
    Occupation: m.occupation || '', Education: m.education || '', BaptismName: m.baptismName || '',
    MaritalStatus: m.maritalStatus || 'Single', Notes: m.notes || ''
  };
}
...
function renderStepBody(body, footer, navigate, state) {
  ...
  else if (state.step === 2) renderStep2(body, state);
  else if (state.step === 3) {
    const rerenderStep3 = () => { renderMembersStep(body, state, () => { saveLocalDraft(state); rerenderStep3(); }); };
    rerenderStep3();
  } else if (state.step === 4) renderReview(body, state, (jumpStep) => { state.step = jumpStep; renderShell(state.root, navigate, state); });
...
function validateStep(state) {
  ...
  if (state.step === 1) {
    if (!f.HouseNumber.trim()) { toast('House Number is required.', 'warning'); return false; }
    ...
    if (!isValidEmail(f.Email)) { toast('Email looks invalid.', 'warning'); return false; }
  }
  return true;
}
...
function renderStep1(body, state) {
  ...
  body.appendChild(readonly('Family ID', f.FamilyID || 'Will be generated on save'));
  grid(body, [
    input('House Number *', f, 'HouseNumber'),
    input('Family Name *', f, 'FamilyName'),
    input('Head of Family', f, 'HeadOfFamily'),
    input('Ward', f, 'Ward'),
    input('Area', f, 'Area'),
    input('House Name', f, 'HouseName'),
    input('Address', f, 'Address'),
    input('Landmark', f, 'Landmark'),
    input('PIN Code', f, 'PinCode'),
...
function renderStep2(body, state) {
  const f = state.family;
  body.appendChild(el('h2', { class: 'step-title' }, ['Location']));
  grid(body, [
    input('District', f, 'District'),
    input('State', f, 'State'),
    input('Country', f, 'Country'),
    input('Google Map Link (optional)', f, 'MapLink', 'url')
  ]);
}
...
function renderReview(body, state, jumpTo) {
  ...
  body.appendChild(reviewSection('Family Details', 1, jumpTo, [
    ['House Number', f.HouseNumber], ['Family Name', f.FamilyName], ['Head of Family', f.HeadOfFamily],
    ['Ward', f.Ward], ['Area', f.Area], ['House Name', f.HouseName], ['Address', f.Address],
    ['Landmark', f.Landmark], ['PIN Code', f.PinCode], ['Phone', f.Phone], ['WhatsApp', f.WhatsApp],
    ['Email', f.Email], ['Remarks', f.Remarks]
  ]));
  body.appendChild(reviewSection('Location', 2, jumpTo, [
    ['District', f.District], ['State', f.State], ['Country', f.Country], ['Map Link', f.MapLink]
  ]));

  const membersBox = el('div', { class: 'review-section' }, [
    el('div', { class: 'review-section__header' }, [
      el('h3', {}, [`Family Members (${state.members.length})`]),
      el('button', { class: 'btn btn--ghost btn--small', onclick: () => jumpTo(3) }, ['Edit'])
    ])
  ]);
...
async function renderSubmitStep(body, footer, navigate, state, doSave) {
  ...
  try {
    ...
    footer.appendChild(el('button', {
      class: 'btn btn--primary', onclick: () => { state.step = 4; renderShell(state.root, navigate, state); }
    }, ['Back to Review']));
  }
}
