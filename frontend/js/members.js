/**
 * members.js
 * Rendering + interaction for the Family Members step (step 3) of the wizard:
 * add/edit/delete/reorder an unlimited list of members as inline expandable cards.
 */

import { el, calcAge, isValidPhone, isValidEmail } from './utils.js';
...
function blankMember() {
  return {
    MemberID: '', MemberName: '', Gender: 'Male', Relation: 'Head', DOB: '', Age: '',
    WeddingDate: '', BloodGroup: '', Phone: '', WhatsApp: '', Email: '', Occupation: '',
    Education: '', BaptismName: '', MaritalStatus: 'Single', Notes: ''
  };
}
...
function renderMemberForm(member, onChange) {
  ...
  form.appendChild(selectField('Gender', member, 'Gender', GENDERS));
  ...
  form.appendChild(textField('Email', member, 'Email', { type: 'email', validate: isValidEmail, errorMsg: 'Invalid email' }));
  form.appendChild(textField('Occupation', member, 'Occupation'));
  form.appendChild(textField('Education', member, 'Education'));
  form.appendChild(textField('Baptism Name', member, 'BaptismName'));
  form.appendChild(selectField('Marital Status', member, 'MaritalStatus', MARITAL_STATUSES));
  form.appendChild(textAreaField('Special Notes', member, 'Notes'));
